...
SecureDrop Workstation's virtual machines are powered by `Debian <https://www.debian.org/>`_ and `Fedora <https://fedoraproject.org/>`_ both of which rely on volunteer contributions and financial support. The `GNOME <https://www.gnome.org/>`_ project acts as an umbrella for many of the individual software components we rely on.
