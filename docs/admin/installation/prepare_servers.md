...
Before beginning the installation process, you should upgrade your servers' BIOS to the most recent stable version available. This process will differ for each server make/model - if you are using one of the recommended NUC models, you can find instructions in [](../maintenance/bios_server.md).
...
Our [specific hardware recommendations](#specific-hardware-recommendations) enumerate recommended BIOS settings for hardware that we have tested.
...
The SecureDrop Application Server and Monitor Server run **Ubuntu Server 24.04.3 LTS (Noble Numbat)**. To install Ubuntu on the servers, [you must first download and verify the Ubuntu installation media.](/admin/installation/prepare_installation_media.md)
