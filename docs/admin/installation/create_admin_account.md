...
In order for any Administrators to access the [Admin Interface](#glossary_admin_interface), they need:
...
You must create the first admin account on the Admin Interface by running a command on the [Application Server](#glossary_application_server). After that, the Administrator can create additional accounts through the web-based Admin Interface.

(create-admin-cli)=

To create an admin account via the command line, [SSH to the Application Server](../installation/test_the_installation.md), then:
...
Once that's done, you should open Tor Browser ![TorBrowser](../../images/torbrowser.png) and navigate to the Admin Interface's onion address. Verify that you can log in to the Admin Interface with the admin account you just created.

For adding more user accounts, please refer now to our [Admin Interface Guide](/admin/reference/admin_interface.md).
