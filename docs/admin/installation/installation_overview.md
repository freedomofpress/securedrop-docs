...
Once you have all the necessary hardware, [setting up SecureDrop](install.md) will take at least a day's work. After installation, you will need at least one more day to [complete and test](#deployment) your setup.
...
As part of [priority support agreements](https://securedrop.org/priority-support/) and on a pro-bono basis for smaller news organizations, Freedom of the Press Foundation will visit your offices, help set up SecureDrop and train *Journalists* to use it. (For pro-bono support, we request that our travel costs are covered.)

```{include} ../../includes/provide-feedback.md
```
...
- Admin Workstation: A laptop running the Qubes OS operating system configured as an Admin Workstation, that you use to install and administer SecureDrop on the servers via SSH. If necessary (i.e. in a small newsroom), the same laptop may also serve as a [SecureDrop Workstation](#glossary_securedrop_workstation) where Journalists decrypt, view, and export messages and files sent by Sources. For a larger newsroom, you may set up additional SecureDrop Workstations as needed for Journalist use.
- Application Server: An Ubuntu server running two segmented Tor hidden services. The Source connects to the Source Interface, a public-facing Tor Onion Service, to send messages and submit files to the Journalist. The Journalist connects to the Admin Interface, an [authenticated Tor Onion Service](https://community.torproject.org/onion-services/advanced/client-auth/), using SecureDrop Inbox on a SecureDrop Workstation to download encrypted messages and files and respond to Sources.
- Monitor Server: An Ubuntu server that monitors the Application Server with [OSSEC](https://www.ossec.net/) and sends email alerts.
- Network Firewall A hardware firewall dedicated to your SecureDrop installation.

A summary of the major steps is as follow:

01. Acquire compatible hardware.
02. Prepare email accounts and GPG keys for alert emails.
03. Prepare an Admin Workstation laptop.
04. Generate the Submission Key.
05. Set up the KeePassXC password manager on the Admin Workstation.
06. Install and configure the dedicated network firewall from the Admin Workstation.
07. Prepare the (Application and Monitor) servers.
08. Install SecureDrop on the servers from the Admin Workstation.
09. Complete local configuration of the Admin Workstation.
10. Create the first Administrator user.
11. Test the installation.

Optionally:

1. Prepare additional Journalist Workstations for use by Journalists.
2. Prepare encrypted Export Devices.
3. Troubleshoot any issues that occurred during installation.
...
A SecureDrop Workstation contains a copy of the [Submission Private Key](#glossary_submission_key) together with messages and files received from Sources in both encrypted and decrypted form. It's critical to ensure that appropriate security practices are applied to a SecureDrop Workstation.
...
- The Application and Monitor Servers should be dedicated physical machines, not virtual machines, [nor hosted on cloud servers](#faq_physically_hosted)..
...
[securedrop installation worksheet]: https://docs.google.com/a/freedom.press/document/d/18RMAzhx1XCgpmw366I8tItBXQTzkFy_i_D0c605DTS8/edit?usp=sharing
