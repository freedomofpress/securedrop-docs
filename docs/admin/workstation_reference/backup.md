...
Qubes OS has a [backup utility](https://www.qubes-os.org/doc/backup-restore/) that allows for backup and restoration of user-specified qubes and templates.
...
- a [LUKS-encrypted](https://workstation.securedrop.org/en/stable/admin/reference/provisioning_usb.html) USB or LUKS-encrypted external hard drive (of sufficient size, if backing up additional local data)
...
In a `dom0` terminal opened via ![Qubes Application menu](../../images/qubes_menu.png) **▸** ![System Tools](../../images/qubes_menu_gear.png) **▸ Other Tools ▸ Xfce Terminal**:
...
Navigate to ![Qubes Application menu](../../images/qubes_menu.png) **▸** ![System Tools](../../images/qubes_menu_gear.png) **▸ Qubes Tools ▸ Backup Qubes**, and move all qubes from "Selected" to "Available" by pressing the `<<` button.
...
To restore SecureDrop Workstation, follow our [pre-install tasks](/admin/installation/prepare_sdw.md) to provision a Qubes OS system complete with updated base templates.
...
Example: If you wish to restore the `vault` qube, rename or delete the existing `vault` qube prior to restoring the backup. You can do so in ![Qubes Application menu](../../images/qubes_menu.png) **▸ Apps ▸ vault ▸ Settings** (the qube must not be running).
...
Then, [download and verify](#download_rpm) the SecureDrop Workstation .rpm to the `work` qube and copy it to `dom0`.
