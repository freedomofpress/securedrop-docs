...
Before troubleshooting connection problems, we recommend reading about the [networking architecture](#networking-architecture) of SecureDrop Workstation. If you are in a hurry, this guide offers quick diagnostic and remedial steps.
...
![screenshot_network_manager_wired_icon](../../images/screenshot_network_manager_wired_icon.png)

It looks like this for a wireless connection:

![screenshot_network_manager_wireless_icon](../../images/screenshot_network_manager_wireless_icon.png)

It looks like this when you are not connected to the Internet at all:

![screenshot_network_manager_no_internet_connection](../../images/screenshot_network_manager_no_internet_connection.png)

When a network connection is lost, Qubes will display an alert like the following:

![screenshot_network_manager_lost_connection_notification](../../images/screenshot_network_manager_lost_connection_notification.png)
...
:::{important}
Not all qubes in Qubes OS have Internet access. For example, opening a terminal via ![Qubes Application menu](../../images/qubes_menu.png) **▸** ![System Tools](../../images/qubes_menu_gear.png) **▸ Other Tools ▸ Xfce Terminal** opens a `dom0` terminal without Internet access. See our [networking architecture](#networking-architecture) overview for additional background.
:::
...
![screenshot_q_widget_sysnet_run_terminal](../../images/screenshot_q_widget_sysnet_run_terminal.png)

1. Click the Qubes Domains menu ![Qubes Domains menu](../../images/blue_qube.png) in the in the system tray (top right area).
...
You can verify whether a qube is running or not by clicking the ![Qubes Domains menu](../../images/blue_qube.png) icon in the system tray (top right). Only qubes that are currently running will appear in the list:

![screenshot_q_widget_vm_list](../../images/screenshot_q_widget_vm_list.png)

If a required qube is not running, you can launch it from the Qube Manager. Open the Qube Manager by clicking **Open Qube Manager** in the menu above. A window like the following should appear:

![screenshot_qube_manager](../../images/screenshot_qube_manager.png)

To start a qube, select it from the list, right-click it, and click **Start/Resume Qube**. Alternatively, you can click the "Play" button in the toolbar.

![screenshot_start_resume_qube](../../images/screenshot_start_resume_qube.png)

In ordinary use, qubes required by SecureDrop should be started on boot or when they are needed. If you repeatedly experience problems with a necessary qube not running, or if an error message is displayed when attempting to start the qube, please contact us for assistance.
...
2. Click the Qubes Application menu ![Qubes Application menu](../../images/qubes_menu.png) icon in the system tray (top left).
...
You may wish to examine system logs on your own, or with our guidance. You can examine consolidated syslogs from all SecureDrop-related qubes in the `sd-log` qube. They can be found in the default user's `~/QubesIncomingLogs` directory.

In addition, you may want to examine `/var/log/syslog` in `sys-net` and `sys-firewall`.

(troubleshooting_admin_connectivity)=
...
2. **Ensure your servers are online.** Visit the Admin Interface to check your Application Server is online, and you can trigger a [test OSSEC alert](#test-ossec-alert) to verify your Monitor Server is online.
