...
## [Application Server](#glossary_application_server) logs

See the directory `/var/log/apache2/*` for web server access and error logs. In production systems, logging is only enabled for the [Admin Interface](#glossary_admin_interface) to the files `journalist-access.log` and `journalist-error.log`, and the logs do not contain IP address information.

When investigating an application error on the [Source Interface](#glossary_source_interface) (e.g., if you see an "Internal Server Error" when sending a message or file), it can make sense to temporarily enable error logging. To do so:
...
Note that the `debug` logging level is highly verbose; if you want to adjust it, see [the Apache documentation](https://httpd.apache.org/docs/2.4/mod/core.html#loglevel) for more information about the different logging levels.

If you encounter an application error, and you have not modified the application code, please be sure to [file an issue](https://github.com/freedomofpress/securedrop/issues/new/) or contact us via <securedrop@freedom.press> ([GPG encrypted](https://securedrop.org/sites/default/files/fpf-email.asc)).

## [Monitor Server](#glossary_monitor_server) logs
